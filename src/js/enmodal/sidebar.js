class Sidebar {
  ...
  add_to_line_selector(line) {
    ...
    $("#dropdown-line-menu").prepend(
      '<li class="line-selector-item"><a class="line-selector-option" transit-line-id="' +
        line.sid.toString() +
        '" href="#"> <div class="subway-line-long" style="background-color: ' +
        line.color_bg +
        "; color: " +
        line.color_fg +
        ';"><div class="content">' +
  ...
  new_line_name() {
    ...
    var used_names = [];
    if (enmodal.transit_interface.active_service) {
      for (
        var i = 0;
        i < enmodal.transit_interface.active_service.lines.length;
        i++
      ) {
  ...
  line_selector_new() {
    if (!enmodal.transit_interface.active_service) return;
  ...
  clear_line_selector() {
    ...
    $("#dropdown-line-button").html(
      'Select a line... <span class="caret"></span>',
    );
  ...
  update_line_selector(id) {
    ...
    $("#dropdown-line-button").html(
      '<div class="subway-line-long" style="background-color: ' +
        line.color_bg +
        "; color: " +
        line.color_fg +
        ';"><div class="content">' +
  ...
  refresh_line_editor() {
    ...
    if (enmodal.transit_interface.active_line !== null) {
      $("#custom-line-options #color-picker-bg").spectrum(
        "set",
        enmodal.transit_interface.active_line.color_bg,
      );
      $("#custom-line-options #color-picker-fg").spectrum(
        "set",
        enmodal.transit_interface.active_line.color_fg,
      );
    } else {
      $("#custom-line-options #color-picker-bg").spectrum(
        "set",
  ...
  line_editor_save() {
    ...
    if (custom_line_name !== undefined)
      custom_line_name = custom_line_name.substring(0, 40);
    ...
    if (!issue) {
      ...
      this.update_line_selector(line.sid);
      $("a[transit-line-id='" + line.sid.toString() + "']").html(
        '<div class="subway-line-long" style="background-color: ' +
          line.color_bg +
          "; color: " +
          line.color_fg +
      ...
      enmodal.transit_interface.draw_line(
        line,
        false,
        true,
        enmodal.transit_interface.layers.active.line_paths,
        true,
        enmodal.transit_interface.active_service,
      );
    } else {
      $("#option-section-lines").animate(
        { scrollTop: $("#option-section-lines").prop("scrollHeight") },
        1000,
      );
    }
  }
    ...
    for (var i = 0; i < stop_groups.length; i++) {
      ...
      for (j = start_index; j < stop_group.length; j++) {
        ...
        if (branch_pos.indexOf(stop.sid) > -1) {
          ...
          $("#route-diagram").append(branch_div);
          var connector_div = $(
            '<div class="route-diagram-branch-connectors"></div>',
          );
          branch_div.append(connector_div);
          connector_div.append(
            '<div class="route-diagram-connector-top-joint" style="background-color: ' +
              line.color_bg +
              ';"></div>',
          );
          connector_div.append(
            '<div class="route-diagram-connector-branch"><div class="route-diagram-connector-internal" style="background-color: ' +
              line.color_bg +
              ';"></div></div>',
          );
          connector_div.append(
            '<div class="route-diagram-connector-bottom-joint" style="background-color: ' +
              line.color_bg +
              ';"></div>',
          );
        }
        ...
        if (j != stop_group.length - 1) {
          stop_div.append(
            '<div class="route-diagram-connector" style="background-color: ' +
              line.color_bg +
              '"></div>',
          );
        }
        ...
        var stop_info_div = $(
          '<div class="route-diagram-stop-info" id="station-' +
            stop.station.sid.toString() +
            '"></div>',
        );
        stop_div.append(stop_info_div);
        stop_info_div.append(
          '<div class="route-diagram-stop-name">' +
            stop.station.name +
            "</div>",
        );

        var stop_connectors = $(
          '<div class="route-diagram-stop-connectors"></div>',
        );
        stop_info_div.append(stop_connectors);
        // Add an empty connector just to make sure each stop row has the height it needs
        stop_connectors.append(
          '<div class="subway-line-long subway-line-mini subway-line-marker-diagram subway-line-marker-diagram-fake" style="font-size: 1em;"><div class="content"></div></div>',
        );
        var connecting_lines = [];
        for (
          var k = 0;
          k < enmodal.transit_interface.active_service.lines.length;
          k++
        ) {
          if (
            enmodal.transit_interface.active_service.lines[k].sid != line.sid
          ) {
            if (
              enmodal.transit_interface.active_service.lines[k].has_station(
                stop.station,
              )
            ) {
              connecting_lines.push(
                enmodal.transit_interface.active_service.lines[k],
              );
            }
          }
        }
        for (
          k = 0;
          k < enmodal.transit_interface.active_service.transfers.length;
          k++
        ) {
          if (
            enmodal.transit_interface.active_service.transfers[k].has_station(
              stop.station,
            )
          ) {
            var transfer_stations =
              enmodal.transit_interface.active_service.transfers[k].stations;
            for (var l = 0; l < transfer_stations.length; l++) {
              if (transfer_stations[l] != stop.station) {
                var transfer_station_lines =
                  enmodal.transit_interface.active_service.station_lines(
                    transfer_stations[l],
                  );
                for (var m = 0; m < transfer_station_lines.length; m++) {
                  if (
                    connecting_lines.indexOf(transfer_station_lines[m]) == -1
                  ) {
        ...
        for (k = 0; k < connecting_lines.length; k++) {
          stop_connectors.append(
            '<div class="subway-line-long subway-line-mini subway-line-marker-diagram" style="font-size: 1em; background-color: ' +
              connecting_lines[k].color_bg +
              "; color: " +
              connecting_lines[k].color_fg +
              ';"><div class="content">' +
              connecting_lines[k].name +
              "</div></div>",
          );
        }
  ...
  add_to_service_selector(service) {
    ...
    $("#dropdown-service-menu").prepend(
      '<li class="service-selector-item"><a class="service-selector-option" transit-service-id="' +
  ...
  service_selector_new() {
    ...
    enmodal.transit_interface.draw_service(
      service,
      enmodal.transit_interface.layers.active,
      true,
      true,
    );
  ...
  service_editor_save() {
    ...
    if (!issue) {
      ...
      $("#option-section-services").animate(
        { scrollTop: $("#option-section-services").prop("scrollHeight") },
        1000,
      );
    }
  }
