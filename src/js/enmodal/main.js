function handle_map_click(e) {
  if (
    ...
    var pp = enmodal.transit_interface.pin_projection(
      e.latlng.lat,
      e.latlng.lng,
    );
    if (!pp[0]) {
  ...
  if (
    ...
    L.popup()
      .setLatLng(e.latlng)
      .setContent("Create a line to start building")
      .openOn(enmodal.leaflet_map);
  }
...
function set_basemap_style(basemap) {
  ...
  if (url) {
    enmodal.transit_interface.basemap = L.tileLayer(url, {
...
function set_basemap_labels(basemap, s) {
  ...
  if (s) {
    ...
    if (url) {
      // Use shadowPane (z-index 500) to ensure labels are above lines (overlayPane 400) but below markers (markerPane 600)
      enmodal.transit_interface.basemapLabels = L.tileLayer(url, {
        subdomains: "abcd",
        maxZoom: 19,
        detectRetina: true,
        pane: "shadowPane",
...
function init_document() {
  ...
  $(document).on("click", ".station-name", function () {
    if ($(this).has("input").length === 0) {
      ...
      var sn = $(this);
      $(this).text("");
      $('<input type="text" class="station-name-edit enm-editable"></textarea>')
        .appendTo($(this))
        .val(text)
        .select()
        .blur(function () {
  ...
  $(document).on("click", ".route-diagram-stop-info", function () {
    ...
    var station_marker =
      enmodal.transit_interface.get_station_marker_by_station(station);
  ...
  $(document).on("click", ".line-selector-option", function (e) {
    enmodal.sidebar.update_line_selector(
      parseInt(e.currentTarget.getAttribute("transit-line-id")),
    );
  });
  ...
  if (SERVICE_MODES_ENABLED) {
    $(".service-mode-button").click(function () {
      ...
      enmodal.sidebar.update_service_selector(
        enmodal.transit_interface.active_service.sid,
        true,
      );
    });
  }
  ...
  $(document).on("click", ".service-selector-option", function (e) {
    enmodal.sidebar.update_service_selector(
      parseInt(e.currentTarget.getAttribute("transit-service-id")),
      true,
    );
  });
  ...
  $("#city-picker-input").autocomplete({
    source: function (request, response) {
      if (is_latlng(request.term)) {
        ...
        $.ajax({
          url:
            "http://search.mapzen.com/v1/autocomplete?api_key=mapzen-t6h4cff&layers=locality&text=" +
            request.term,
          dataType: "json",
    ...
    select: function (event, ui) {
      ...
      enmodal.transit_interface.map.panTo(
        L.latLng(ui.item.value.coordinates[1], ui.item.value.coordinates[0]),
      );
      return false;
    },
    minLength: 3,
  ...
  $(document).on("click", "#map-title-inner", function () {
    if ($(this).has("input").length === 0) {
      ...
      var sn = $(this);
      $(this).text("");
      $('<input type="text" class="map-title-edit enm-editable"></input>')
        .appendTo($(this))
        .val(text)
        .select()
        .blur(function () {
