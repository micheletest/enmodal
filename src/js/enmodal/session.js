  ...
  $.ajax({
    ...
    success: function (data, status) {
      if (!data || !data.private_key) {
        console.error(
          "Session creation failed: No private key returned from server.",
        );
        return;
      }
...
function handle_map_data(jdata) {
  ...
  if (enmodal.transit_map.services.length === 0) {
    ...
    enmodal.transit_interface.active_service =
      enmodal.transit_map.primary_service();
    enmodal.transit_interface.active_line =
      enmodal.transit_map.primary_service().lines[0];
    enmodal.sidebar.update_service_selector(
      enmodal.transit_interface.active_service.sid,
      true,
    );
    ...
    enmodal.sidebar.update_line_selector(
      enmodal.transit_interface.active_line.sid,
    );
    ...
    for (var i = 0; i < user_settings.station_pairs.length; i++) {
      ...
      var sp_real = enmodal.transit_interface.get_station_pair(
        station_1,
        station_2,
      );
      if (sp_real !== null) {
...
function session_load() {
  return new Promise((resolve, reject) => {
    ...
    $.ajax({
      url: "session_load?" + params,
      async: true,
      error: function (jqXHR, textStatus, errorThrown) {
        console.error("Session load failed: ", textStatus, errorThrown);
        alert("Error loading session. Please check server logs.");
        reject(errorThrown);
      },
      success: function (data, status) {
        //console.log(data);
